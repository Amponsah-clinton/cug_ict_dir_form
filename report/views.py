...
def _send_notification(director_name, signature_date, submission_id):
    ...
    text = (
        f"The Independent Expert / Technical Arbiter Confirmation Form has been submitted.\n\n"
        f"Independent Expert / Technical Arbiter: {director_name or '(not entered)'}\n"
    ...
    <div style="background:#f9f9f9;padding:24px 30px;border:1px solid #ddd;border-top:none;border-radius:0 0 6px 6px">
      <p>The Independent Expert / Technical Arbiter Confirmation Form has been filled and saved.</p>
      <table style="border-collapse:collapse;width:100%;margin:16px 0">
        <tr><td style="padding:6px 0;font-weight:bold;width:120px">Independent Expert / Technical Arbiter</td>
...
# ── API: save Independent Expert / Technical Arbiter's answer (yes_no / true_false / text types) ──

@require_http_methods(['POST'])
def save_answer(request):
    """Persist the Independent Expert / Technical Arbiter's typed/selected answer for a non-checkbox item."""
    try:
        data          = json.loads(request.body)
        correction_id = data.get('correction_id', '').strip()
...
def save_confirmation(request):
    try:
        ...
        if not name:
            return JsonResponse({'error': 'name_required', 'message': 'Independent Expert / Technical Arbiter name is required.'}, status=400)
